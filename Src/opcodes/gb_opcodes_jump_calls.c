...
uint8_t opcode_jp(uint16_t opcode_address){
    ...
    CPU_REG.PC = op_get_mem_map_16(opcode_address+1);
...
uint8_t opcode_jr(uint16_t opcode_address){
    uint8_t opcode = op_get_mem_map_8(opcode_address);
    int8_t temp = (int8_t)op_get_mem_map_8(opcode_address+1);
    CPU_REG.PC += temp;
    DEBUG_PRINT("%s: CPU_REG.PC = %04x\n",__func__,CPU_REG.PC);
    return opcode_table[opcode].cycles;
}
...
uint8_t opcode_call(uint16_t opcode_address){
    uint8_t opcode = op_get_mem_map_8(opcode_address);
    CPU_REG.SP -= 2;
    op_set_mem_map_16(CPU_REG.SP,CPU_REG.PC);
    CPU_REG.PC = op_get_mem_map_16(opcode_address+1);
    DEBUG_PRINT("%s: CPU_REG.PC = %04x\n",__func__,CPU_REG.PC);
    return opcode_table[opcode].cycles;
}
...
uint8_t opcode_ret(uint16_t opcode_address){
    uint8_t opcode = op_get_mem_map_8(opcode_address);
    CPU_REG.PC = op_get_mem_map_16(CPU_REG.SP);
    CPU_REG.SP += 2;
    DEBUG_PRINT("%s: CPU_REG.PC = %04x\n",__func__,CPU_REG.PC);
    return opcode_table[opcode].cycles;
}
